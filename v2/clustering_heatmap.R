...
ggplot(elbow, aes(x=cluster, y= value)) + 
  ...
  ggsave("D://deus_ex_machina/v2/elbow_graph.png")
...
xtab %>% 
  ...
  ggsave("D://deus_ex_machina/v2/cluster_heatmap.png", width = 6)

mean <- as.data.frame(k$centers)

write_csv(mean, "D://deus_ex_machina/v2/mean_table.csv")
...
graph %>% 
  ...
  annotate("text", x=4.3, y = 3.6, label = "Sample Mean", size = 6, family = "font") +
  theme(legend.title=element_blank()) +
  theme(text=element_text(size=28, family="font")) +
  ggsave("D://deus_ex_machina/v2/pid_means.png", height = 4, width =16) 

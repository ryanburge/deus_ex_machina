...
## Patriotic or Violent ####

anes <- anes %>% 
  mutate(muslim_violent = car::recode(V162353, "-9:-1=NA")) %>% 
  mutate(muslim_violent = muslim_violent/7) %>% 
  mutate(xtn_violent = car::recode(V162354, "-9:-1=NA")) %>% 
  mutate(xtn_violent = xtn_violent/7) %>% 
  mutate(muslim_patriot = car::recode(V162355, "-9:-1=NA")) %>% 
  mutate(muslim_patriot = muslim_patriot/7) %>% 
  mutate(xtn_patriot = car::recode(V162356, "-9:-1=NA")) %>% 
  mutate(xtn_patriot = xtn_patriot/7)
